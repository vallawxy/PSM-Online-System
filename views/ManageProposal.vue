        ...
        <table class="table m-0">
          ...
          <tbody>
            <tr v-for="proposal in proposalList" :key="proposal.key">
              <td width="20%" v-if="checkedSession.includes(proposal.student.session) || checkedSemester.includes(proposal.student.semester) || displayAll">{{ proposal.student.name }}</td>
              <td width="20%" v-if="checkedSession.includes(proposal.student.session) || checkedSemester.includes(proposal.student.semester) || displayAll">{{ proposal.student.session }}</td>
              <td width="20%" v-if="checkedSession.includes(proposal.student.session) || checkedSemester.includes(proposal.student.semester) || displayAll">{{ proposal.student.semester }}</td>
              ...
              <router-link :to="`/CommView/${proposal.key}`">
              ...
              <!-- <td width="15%" v-if="checkedSession.includes(student.session) || checkedSemester.includes(student.semester) || displayAll">{{ title }}</td>
              <td width="15%" v-if="checkedSession.includes(student.session) || checkedSemester.includes(student.semester) || displayAll">{{ project }}</td> -->
              <td width="2%" v-if="checkedSession.includes(proposal.student.session) || checkedSemester.includes(proposal.student.semester) || displayAll">
                <div class="w-100 btn-group" role="group">
                  <router-link :to="`/viewProposal/${proposal.key}`">
              ...
              <td width="15%">{{ semester }}</td> -->
              <!-- <td width="10%" v-if="checkedSession.includes(student.session) || checkedSemester.includes(student.semester) || displayAll">
                <div class="w-100 btn-group" role="group">
                  <button class="btn btn-success mx-1" @click="download(url)">
                    Download
                  </button>
                </div>
              </td> -->
